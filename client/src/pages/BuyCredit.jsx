  ...
  const initPay = async (order) => {
    const options = {
      ...
      handler: async (response) => {
        try {
          const respo = await axios.post(backendUrl + '/api/user/pay-razor', response, {headers: {token}})
    ...
    <motion.div
      ...
      <div className='flex flex-wrap justify-center gap-7 text-left'>
        {plans.map((item, index)=>(
          <div 
            key={index}
            className='bg-white drop-shadow-sm rounded-lg py-16 px-12 text-gray-600 hover:scale-105 transition-all duration-500'>
